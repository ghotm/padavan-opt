...
nvramshow=`nvram show | grep meow | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow
...
if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep meow)" ] ; then
	mkdir -p /tmp/script
...
if [ "$meow_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "$(ps - w | grep "$meow_path" | grep -v grep )" ] && logger -t "【meow】" "停止 meow"
	meow_close
