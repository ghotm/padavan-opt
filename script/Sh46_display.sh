...
nvramshow=`nvram show | grep display | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow

[ -z $display_enable ] && display_enable=0 && nvram set display_enable=0

if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep display)" ] ; then
	mkdir -p /tmp/script
...
if [ "$display_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "`pidof lcd4linux`" ] && logger -t "【相框显示】" "停止 lcd4linux"
	display_close
