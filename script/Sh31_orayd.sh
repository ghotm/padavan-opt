...
#nvramshow=`nvram show | grep phddns | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow
...
if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep orayd)" ] ; then
	mkdir -p /tmp/script
...
phddns_check () {
if [ "$phddns" != "1" ] ; then
	[ ! -z "`pidof oraysl`" ] && logger -t "【花生壳内网版】" "停止 oraysl"
	phddns_close
