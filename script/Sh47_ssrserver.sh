...
nvramshow=`nvram show | grep ssrserver | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow

[ -z $ssrserver_enable ] && ssrserver_enable=0 && nvram set ssrserver_enable=0

if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep ssrserver)" ] ; then
	mkdir -p /tmp/script
...
if [ "$ssrserver_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "`ps - w | grep manyuser/shadowsocks/server | grep -v grep `" ] && logger -t "【SSR_server】" "停止 ssrserver"
	ssrserver_close
