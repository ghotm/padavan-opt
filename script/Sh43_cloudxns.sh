...
nvramshow=`nvram show | grep cloudxns | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow
hostIP=""
IP=""
...
if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep cloudxns)" ] ; then
	mkdir -p /tmp/script
...
if [ "$cloudxns_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "$(ps - w | grep "$scriptname keep" | grep -v grep )" ] && logger -t "【CloudXNS动态域名】" "停止 cloudxns"
	cloudxns_close
