...
nvramshow=`nvram show | grep xunlei | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow

[ -z $xunleis ] && xunleis=0 && nvram set xunleis=0

if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep xun_lei)" ] ; then
	mkdir -p /tmp/script
...
if [ "$xunleis" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "`pidof ETMDaemon`" ] && logger -t "【迅雷下载】" "停止 xunleis"
	xunlei_close
