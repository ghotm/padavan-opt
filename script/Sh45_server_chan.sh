...
nvramshow=`nvram show | grep serverchan | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow

[ -z $serverchan_enable ] && serverchan_enable=0 && nvram set serverchan_enable=0

if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep server_chan)" ] ; then
	mkdir -p /tmp/script
...
if [ "$serverchan_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "`pidof serverchan`" ] && logger -t "【微信推送】" "停止 serverchan"
	serverchan_close
