...
nvramshow=`nvram show | grep ssserver | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow
...
if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep ssserver)" ] ; then
	mkdir -p /tmp/script
...
if [ "$ssserver_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "`pidof ss-server`" ] && logger -t "【SS_server】" "停止 ss-server"
	ssserver_close
