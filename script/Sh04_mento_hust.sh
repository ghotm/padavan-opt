...
nvramshow=`nvram show | grep mentohust | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow
...
if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep mento_hust)" ] ; then
	mkdir -p /tmp/script
...
if [ "$mentohust_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "`pidof mentohust`" ] && logger -t "【MentoHUST】" "停止 mentohust"
	mentohust_close
