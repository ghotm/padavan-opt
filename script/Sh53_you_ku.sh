...
nvramshow=`nvram show | grep youku | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow

[ -z $youku_enable ] && youku_enable=0 && nvram set youku_enable=0

if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep you_ku)" ] ; then
	mkdir -p /tmp/script
...
if [ "$youku_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "`pidof ikuacc`" ] && logger -t "【路由宝】" "停止 youku"
	youku_close
