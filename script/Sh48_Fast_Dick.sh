...
nvramshow=`nvram show | grep FastDick | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow

[ -z $FastDick_enable ] && FastDick_enable=0 && nvram set FastDick_enable=0

if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep Fast_Dick)" ] ; then
	mkdir -p /tmp/script
...
if [ "$FastDick_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	...
	[ $running -gt 1 ] && logger -t "【迅雷快鸟】" "停止 迅雷快鸟$running"
	FastDick_clos
