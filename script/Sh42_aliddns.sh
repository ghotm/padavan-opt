...
nvramshow=`nvram show | grep aliddns | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow
hostIP=""
domain=""
name=""
name1=""
...
if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep aliddns)" ] ; then
	mkdir -p /tmp/script
...
if [ "$aliddns_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "$(ps - w | grep "$scriptname keep" | grep -v grep )" ] && logger -t "【aliddns动态域名】" "停止 aliddns"
	aliddns_close
