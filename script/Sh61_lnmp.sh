...
nvramshow=`nvram show | grep lnmp | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow

if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep lnmp)" ] ; then
	mkdir -p /tmp/script
...
if [ "$default_enable" = "1" ] || [ "$default_enable" = "2" ] ; then
	if [ ! -d "/opt/www/default" ] ; then
		...
		if [ ! -f "/opt/www/default/tz.php" ] ; then
			...
			wgetcurl.sh /opt/www/default/tz.php "$hiboyfile/tz.php"
		fi
	fi
	if [ ! -d "/opt/www/default" ] ; then
