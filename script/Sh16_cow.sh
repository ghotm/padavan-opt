...
nvramshow=`nvram show | grep cow | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow
...
if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep cow)" ] ; then
	mkdir -p /tmp/script
...
if [ "$cow_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "$(ps - w | grep "$cow_path" | grep -v grep )" ] && logger -t "【cow】" "停止 cow"
	cow_close
