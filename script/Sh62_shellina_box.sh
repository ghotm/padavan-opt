...
nvramshow=`nvram show | grep shellinabox | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow

if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep shellina_box)" ] ; then
	mkdir -p /tmp/script
...
if [ "$shellinabox_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "`pidof shellinaboxd`" ] && logger -t "【shellinabox】" "停止 shellinaboxd"
	shellinabox_close
