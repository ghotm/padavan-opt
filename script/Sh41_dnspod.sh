...
nvramshow=`nvram show | grep dnspod | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow
hostIP=""
myIP=""
...
if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep dnspod)" ] ; then
	mkdir -p /tmp/script
...
if [ "$dnspod_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "$(ps - w | grep "$scriptname keep" | grep -v grep )" ] && logger -t "【DNSPod动态域名】" "停止 dnspod"
	dnspod_close
