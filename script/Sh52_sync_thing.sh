...
nvramshow=`nvram show | grep syncthing | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow

[ -z $syncthing_enable ] && syncthing_enable=0 && nvram set syncthing_enable=0

if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep sync_thing)" ] ; then
	mkdir -p /tmp/script
...
if [ "$syncthing_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "`pidof syncthing`" ] && logger -t "【syncthing】" "停止 syncthing"
	syncthing_close
