...
nvramshow=`nvram show | grep ngrok | awk '{print gensub(/=/,"='\''",1,$0)"'\'';";}'` && eval $nvramshow

[ -z $ngrok_enable ] && ngrok_enable=0 && nvram set ngrok_enable=0

if [ ! -z "$(echo $scriptfilepath | grep -v "/tmp/script/" | grep ngrok)" ] ; then
	mkdir -p /tmp/script
...
if [ "$ngrok_enable" != "1" ] && [ "$needed_restart" = "1" ] ; then
	[ ! -z "`pidof ngrokc`" ] && logger -t "【ngrok】" "停止 ngrok"
	ngrok_close
